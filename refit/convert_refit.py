...
def _convert(input_path, store, tz, sort_index=True):
    ...
    for house_id in houses:
        ...
        #usecols = ['Unix', 'Aggregate', 'Appliance1', 'Appliance2', 'Appliance3', 'Appliance4', 'Appliance5',
        #           'Appliance6', 'Appliance7', 'Appliance8', 'Appliance9']

        df = _load_csv(csv_filename, tz)
        if sort_index:
...
def _load_csv(filename, tz):
    ...
    df.loc[0] = pd.to_datetime(df.loc[0], unit='s', utc=True)
...
def main():
    input_path = 'REFITData'
    output_filename = 'refit.h5'
    convert_refit(input_path, output_filename)
